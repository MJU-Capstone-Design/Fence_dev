...
class HelloInfo extends React.PureComponent {
  render() {
    ...
    if (info.crime.length == 0) {
      info.crime.push("5대범죄에서 안전한 지역 입니다!", <br />, "행운아시네요 :)")
    } else {
    ...
    if (info.compare.length == 0) {
      info.compare.push("부족한 안전시설이 없습니다!", <br />, "관할구청에 칭찬의 말 한마디 어떨까요?")
    } else {
    ...
    return (
      <InfoWindow onCloseClick = {this.props.onCloseClick}>
        <div class="infoTag">
          <p>현재 이 지역은 {info.crime} <br />이 지역은 다른 지역들에 비해 {info.compare} <br />연략처 : {info.contact}</p>
        </div>
      </InfoWindow>
