...
app.get("/api/lights", (req, res) => {
  ...
  var queryString = "SELECT * FROM 방범등가로등 limit 200";
...
app.get("/api/cctvs", function (req, res) {
  ...
  var queryString = "SELECT * FROM CCTV limit 200";
  connection.query(queryString, function (err, rows, field) {
    // console.log(rows)
    if (err) throw err;
    res.json(rows);
...
app.get("/api/polices", function (req, res) {
  ...
  var queryString = "SELECT * FROM 치안시설 limit 200";
  connection.query(queryString, function (err, rows, field) {
    // console.log(rows)
    if (err) throw err;
    res.json(rows);
...
app.get("/api/bells", function (req, res) {
  ...
  var queryString = "SELECT * FROM 안전비상벨 limit 200";
  connection.query(queryString, function (err, rows, field) {
    // console.log(rows)
    if (err) throw err;
    res.json(rows);
